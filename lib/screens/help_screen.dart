    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _section('招式模板', [
            _item('模板库页面', '首页右上角书签图标管理「通用模板」；进入人物后点书签图标可见「通用模板 + 本角色专属」两组，模板管理集中在此页面，不再挤占人物页'),
            _item('新建模板', '在模板库页面点 +，或在连段编辑器底部模板区点「新建」（无需退出即可新增并编辑），也可在连段编辑器选区后保存'),
            _item('编辑模板', '点击模板 chip（含颜色圆点）进入编辑页面'),
            _item('通用 vs 专属', '编辑页有「对全角色可见」开关。开启=通用模板（所有角色共享同一份）；关闭=转为某角色专属。本质是在通用库与角色专属之间切换归属'),
            _item('转为专属', '通用模板在模板库页面 chip 右侧人形图标可转为指定角色专属'),
            _item('转为通用', '在招式编辑器底部「本角色」分块里，点模板 chip 右下角的 🌐 图标，可把该角色专属模板直接转为通用模板（对所有角色可见）'),
            _item('使用模板', '在招式编辑器底部"模板"区域分「通用/本角色」两块显示，把模板拖拽到编辑区即可使用。模板很多时可点组标题折叠/展开，单组超过 10 个自动分页（底部翻页）'),
            _item('模板颜色', '编辑模板时从调色板选择颜色；不选默认黑色'),
            _item('PDF 显示名字', '勾选"导出 PDF 时显示模板名而不是具体指令"'),
            _item('模板备注', '模板备注在 PDF 中显示在括号内末尾 * 之后'),
            _item('删除模板', '长按模板 chip 或在模板编辑页点击删除图标（删除通用模板会影响所有角色）'),
          ]),
          _section('PDF 导出', [
          ...
          _section('数据管理', [
            _item('替换导入', '在首页点击导入图标，选择「替换导入」用所选 JSON 文件完全覆盖当前数据'),
            _item('合并导入', '选择「合并导入」可把另一个文件的内容并入当前数据：按 id 合并角色/条目/招式，全局与各角色专属模板按 id 去重，仅新增本地缺失的项，已有数据保留不变'),
            _item('导出数据', '在首页点击导出图标，保存 JSON 备份文件'),
            _item('自动保存', '所有编辑会自动保存到本地，无需手动保存'),
            _item('自动备份', '每次保存前会把上一份存成 sf6_data.json.bak；若主文件损坏，下次启动会自动回退到备份'),
